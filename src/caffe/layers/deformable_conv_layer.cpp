...
void DeformableConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  num_output_ = this->layer_param_.deformable_convolution_param().num_output();
  CHECK_GT(num_output_, 0);
  group_ = this->layer_param_.deformable_convolution_param().group();
  CHECK_EQ(channels_ % group_, 0);
  CHECK_EQ(num_output_ % group_, 0)
      << "Number of output should be multiples of group.";
  deformable_group_ = this->layer_param_.deformable_convolution_param().deformable_group();
  CHECK_EQ(channels_ % deformable_group_, 0)
      << "Number of channels should be multiples of deformable group.";
  CHECK_EQ(num_output_ % deformable_group_, 0)
      << "Number of output should be multiples of deformable group.";
  ...
  bias_term_ = this->layer_param_.deformable_convolution_param().bias_term();
...
void DeformableConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int bottom_id = 2; bottom_id < bottom.size(); bottom_id += 2) {
  ...
  for (int bottom_id = 1; bottom_id < bottom.size(); bottom_id += 2) {
    CHECK_EQ(bottom[bottom_id]->shape(1) % (kernel_shape_data[0] * kernel_shape_data[1]), 0)
        << "offset filter must divide deformable group size";
    CHECK_EQ(bottom[bottom_id]->shape(1) / (2 * kernel_shape_data[0] * kernel_shape_data[1]), deformable_group_)
        << "offset filter must divide deformable group size";
  }
  // Shape the tops.
  for (int top_id = 0; top_id < top.size(); ++top_id) {
    ...
    CHECK_EQ(top_shape[2], bottom[top_id + 1]->shape(2))
        << "output height must equal to offset map height";
    CHECK_EQ(top_shape[3], bottom[top_id + 1]->shape(3))
      << "output height must equal to offset map height";
  }

  reshape_variables(bottom[0], bottom[1], top[0]);
}

template <typename Dtype>
void DeformableConvolutionLayer<Dtype>::reshape_variables(const Blob<Dtype>* bottom, const Blob<Dtype>* offset,
  ...
  offset_dim_ = offset->count(channel_axis_);
...
void DeformableConvolutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < bottom.size(); i += 2) {
    ...
    const Dtype* offset_data = bottom[i + 1]->cpu_data();
    Dtype* top_data = top[i / 2]->mutable_cpu_data();
    if (bottom.size() > 2) {
      reshape_variables(bottom[i], bottom[i + 1], top[i / 2]);
    }
    for (int n = 0; n < num_; ++n) {
      forward_cpu_gemm(bottom_data + n * bottom_dim_, offset_data + n * offset_dim_, weight,
          top_data + n * top_dim_);
      if (bias_term_) {
...
void DeformableConvolutionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i = 0; i < top.size(); ++i) {
    ...
    const Dtype* bottom_data = bottom[2 * i]->cpu_data();
    const Dtype* offset_data = bottom[2 * i + 1]->cpu_data();
    Dtype* bottom_diff = bottom[2 * i]->mutable_cpu_diff();
    Dtype* offset_diff = bottom[2 * i + 1]->mutable_cpu_diff();
    if (top.size() > 1) {
    ...
    if (this->param_propagate_down_[0] || propagate_down[2 * i] || propagate_down[2 * i + 1]) {
      for (int n = 0; n < num_; ++n) {
        // gradient w.r.t. weight. Note that we will accumulate diffs.
        if (this->param_propagate_down_[0]) {
          weight_cpu_gemm(bottom_data + n * bottom_dim_, offset_data + n * offset_dim_,
...
template <typename Dtype>
void DeformableConvolutionLayer<Dtype>::forward_cpu_gemm(const Dtype* input, const Dtype* offset,
    const Dtype* weights, Dtype* output) {
  const Dtype* col_buff = input;
  conv_im2col_cpu(input, offset, col_buffer_.mutable_cpu_data());
...
template <typename Dtype>
void DeformableConvolutionLayer<Dtype>::forward_cpu_bias(Dtype* output,
    const Dtype* bias) {
...
template <typename Dtype>
void DeformableConvolutionLayer<Dtype>::weight_cpu_gemm(const Dtype* input, const Dtype* offset,
...
template <typename Dtype>
void DeformableConvolutionLayer<Dtype>::backward_cpu_bias(Dtype* bias,
    const Dtype* input) {
...
template <typename Dtype>
void DeformableConvolutionLayer<Dtype>::forward_gpu_gemm(const Dtype* input, const Dtype* offset,
    const Dtype* weights, Dtype* output) {
  const Dtype* col_buff = input;
  conv_im2col_gpu(input, offset, col_buffer_.mutable_gpu_data());
...
template <typename Dtype>
void DeformableConvolutionLayer<Dtype>::forward_gpu_bias(Dtype* output, const Dtype* bias) {
...
template <typename Dtype>
void DeformableConvolutionLayer<Dtype>::weight_gpu_gemm(const Dtype* input, const Dtype* offset,
    const Dtype* output, Dtype* weights) {
  const Dtype* col_buff = input;
  conv_im2col_gpu(input, offset, col_buffer_.mutable_gpu_data());
  col_buff = col_buffer_.gpu_data();
...
template <typename Dtype>
void DeformableConvolutionLayer<Dtype>::backward_gpu_bias(Dtype* bias,
    const Dtype* input) {
