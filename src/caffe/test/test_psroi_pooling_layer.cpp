...
class PSROIPoolingLayerTest : public MultiDeviceTest<TypeParam> {
...
TYPED_TEST(PSROIPoolingLayerTest, TestForward) {
  ...
  EXPECT_NEAR(this->blob_top_data_->cpu_data()[0], 4, 1e-5);
  EXPECT_NEAR(this->blob_top_data_->cpu_data()[1], 31, 1e-5);
  EXPECT_NEAR(this->blob_top_data_->cpu_data()[2], 64, 1e-5);
  EXPECT_NEAR(this->blob_top_data_->cpu_data()[3], 91, 1e-5);
}

TYPED_TEST(PSROIPoolingLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  PSROIPoolingParameter* psroi_pooling_param = layer_param.mutable_psroi_pooling_param();
  psroi_pooling_param->set_group_size(2);
